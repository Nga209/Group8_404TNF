...
class AIController extends Controller {
    public function chat() {
        ...
        try {
            ...
            // 4. Thống kê sử dụng phòng/sân (đặt nhiều nhất)
            $stmtStats = $db->query("
                SELECT r.name, c.name as category, COUNT(b.id) as booking_count 
                FROM bookings b 
                JOIN resources r ON b.resource_id = r.id 
                JOIN resource_categories c ON r.category_id = c.id
                ...
                LIMIT 5
            ");
            $dbStats = $stmtStats->fetchAll(PDO::FETCH_ASSOC);
            $contextStats = "";
            if (!empty($dbStats)) {
                $contextStats = "Thống kê đặt phòng/sân nhiều nhất (đã duyệt):\n";
                foreach ($dbStats as $stat) {
                    $contextStats .= "- " . $stat['name'] . " (" . $stat['category'] . "): " . $stat['booking_count'] . " lượt sử dụng thành công\n";
                }
            } else {
                $contextStats = "Chưa có lượt đặt phòng/sân nào được phê duyệt thành công.\n";
            }
            
            // ========================================================
            // B. KIỂM TRA & SỬ DỤNG GEMINI 2.5 FLASH NẾU CÓ API KEY
            // ========================================================
            if (defined('GEMINI_API_KEY') && GEMINI_API_KEY !== '') {
                $systemInstruction = "Bạn là Trợ lý AI thông minh tích hợp trong hệ thống đặt phòng CampusBook của trường học.\n"
                    ...
                    . "📜 CHÍNH SÁCH ĐẶT LỊCH:\n" . $contextPolicies . "\n"
                    . "📊 THỐNG KÊ ĐẶT PHÒNG/SÂN NHIỀU NHẤT:\n" . $contextStats . "\n\n"
                    . "YÊU CẦU TRẢ LỜI:\n"
